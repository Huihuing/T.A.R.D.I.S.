...
public class AccountController {
    ...
    public ResponseEntity<?> transfer(
        ...
        try {
            ...
            String accountPassword = request.get("accountPassword") == null
                    ? null
            ...
            // Both wallets are locked in username order in one query.
            // This prevents lost updates and opposite-direction transfer deadlocks.
            List<Wallet> lockedWallets = walletRepository.findAllForUpdateByUsernames(
                    List.of(fromUsername, toUsername));

            Map<String, Wallet> byUsername = lockedWallets.stream()
                    .collect(Collectors.toMap(
                            wallet -> wallet.getMember().getUsername(),
                            Function.identity()
                    ));
            ...
            if (fromWallet == null) {
                throw new IllegalArgumentException("보내는 사용자의 지갑을 찾을 수 없습니다.");
            }
            if (toWallet == null) {
                throw new IllegalArgumentException("송금받을 유저가 존재하지 않거나 지갑이 없습니다.");
            }
            if (accountPassword == null
                    || !passwordEncoder.matches(accountPassword, fromWallet.getMember().getPin())) {
                return ResponseEntity.badRequest().body(Map.of(
            ...
            return ResponseEntity.status(500).body(
                    Map.of("message", "송금 중 오류가 발생했습니다."));
        }
    }
    ...
    private double parsePositiveAmount(Object value) {
        ...
        try {
            ...
            if (!Double.isFinite(parsed) || parsed <= 0 || parsed > 1_000_000_000.0) {
                throw new IllegalArgumentException("금액을 올바르게 입력하세요.");
            }
